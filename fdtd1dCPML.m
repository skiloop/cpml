...
sigmaOpt=0.8*(pmlOrder+1)/(sqrt(epsR)*dz*Z0);
sigmaMax=sigmaOpt*sigmaMaxDivSigmaOpt;
...
sigma_m=sigmaMax*abs(rho_m).^pmlOrder;
kappa_e=1+(kappaMax-1)*abs(rho_e).^pmlOrder;
kappa_m=1+(kappaMax-1)*abs(rho_m).^pmlOrder;
alpha_e=alphaMax*abs(1-rho_e).^alphaOrder;
alpha_m=alphaMax*abs(1-rho_m).^alphaOrder;
