...
void cpml<T>::createCPMLArrays(unsigned nx, unsigned ny, unsigned nz) {
	...
	if (is_cpml_xn) {
		...
		Psi_hzx_xn.create3DArray(n_cpml_xn, ny, nzp1, 0.0);
		CPsi_eyx_xn.create3DArray(n_cpml_xn, ny, nzp1, 0.0);
		CPsi_ezx_xn.create3DArray(n_cpml_xn, nyp1, nz, 0.0);
		CPsi_hyx_xn.create3DArray(n_cpml_xn, nyp1, nz, 0.0);
		CPsi_hzx_xn.create3DArray(n_cpml_xn, ny, nzp1, 0.0);
	}
	// xp arrays
	if (is_cpml_xp) {
		...
		Psi_hzx_xp.create3DArray(n_cpml_xp, ny, nzp1, 0.0);
		CPsi_eyx_xp.create3DArray(n_cpml_xp, ny, nzp1, 0.0);
		CPsi_ezx_xp.create3DArray(n_cpml_xp, nyp1, nz, 0.0);
		CPsi_hyx_xp.create3DArray(n_cpml_xp, nyp1, nz, 0.0);
		CPsi_hzx_xp.create3DArray(n_cpml_xp, ny, nzp1, 0.0);
	}
	// yn arrays
	if (is_cpml_yn) {
		// y direction
		...
		Psi_hzy_yn.create3DArray(nx, n_cpml_yn, nzp1, 0.0);
		CPsi_exy_yn.create3DArray(nx, n_cpml_yn, nzp1, 0.0);
		CPsi_ezy_yn.create3DArray(nxp1, n_cpml_yn, nz, 0.0);
		CPsi_hxy_yn.create3DArray(nxp1, n_cpml_yn, nz, 0.0);
		CPsi_hzy_yn.create3DArray(nx, n_cpml_yn, nzp1, 0.0);
	}
	// yp arrays
	if (is_cpml_yp) {
		...
		Psi_hzy_yp.create3DArray(nx, n_cpml_yp, nzp1, 0.0);
		CPsi_exy_yp.create3DArray(nx, n_cpml_yp, nzp1, 0.0);
		CPsi_ezy_yp.create3DArray(nxp1, n_cpml_yp, nz, 0.0);
		CPsi_hxy_yp.create3DArray(nxp1, n_cpml_yp, nz, 0.0);
		CPsi_hzy_yp.create3DArray(nx, n_cpml_yp, nzp1, 0.0);
	}
	// zn arrays
	if (is_cpml_zn) {
		// z direction
		...
		Psi_hyz_zn.create3DArray(nx, nyp1, n_cpml_zn, 0.0);
		CPsi_exz_zn.create3DArray(nx, nyp1, n_cpml_zn, 0.0);
		CPsi_eyz_zn.create3DArray(nxp1, ny, n_cpml_zn, 0.0);
		CPsi_hxz_zn.create3DArray(nxp1, ny, n_cpml_zn, 0.0);
		CPsi_hyz_zn.create3DArray(nx, nyp1, n_cpml_zn, 0.0);
	}
	// zp arrays
	if (is_cpml_zp) {
		...
		Psi_hyz_zp.create3DArray(nx, nyp1, n_cpml_zp, 0.0);
		CPsi_exz_zp.create3DArray(nx, nyp1, n_cpml_zp, 0.0);
		CPsi_eyz_zp.create3DArray(nxp1, ny, n_cpml_zp, 0.0);
		CPsi_hxz_zp.create3DArray(nxp1, ny, n_cpml_zp, 0.0);
		CPsi_hyz_zp.create3DArray(nx, nyp1, n_cpml_zp, 0.0);
	}
}
...
template<class T>
void cpml<T>::initCoefficientArrays(short pmlOrder, T alphaOrder, T sigmaMax, T kappaMax, T alphaMax, T epsR, T dt, T dx, T dy, T dz,
	...
	data3d<T>&Ceyhx, data3d<T>&Cexhy, data3d<T>&Chyex, data3d<T>&Chxey) {
		initCoefficientArraysXN(pmlOrder, alphaOrder, sigmaMax, kappaMax, alphaMax, epsR, dt, dx, Ceyhz, Cezhy, Chyez, Chzey);
		initCoefficientArraysXP(pmlOrder, alphaOrder, sigmaMax, kappaMax, alphaMax, epsR, dt, dx, Ceyhz, Cezhy, Chyez, Chzey);
		initCoefficientArraysYN(pmlOrder, alphaOrder, sigmaMax, kappaMax, alphaMax, epsR, dt, dy, Cexhz, Cezhx, Chxez, Chzex);
		initCoefficientArraysYP(pmlOrder, alphaOrder, sigmaMax, kappaMax, alphaMax, epsR, dt, dy, Cexhz, Cezhx, Chxez, Chzex);
		initCoefficientArraysZN(pmlOrder, alphaOrder, sigmaMax, kappaMax, alphaMax, epsR, dt, dz, Ceyhx, Cexhy, Chyex, Chxey);
		initCoefficientArraysZP(pmlOrder, alphaOrder, sigmaMax, kappaMax, alphaMax, epsR, dt, dz, Ceyhx, Cexhy, Chyex, Chxey);
}

template<class T>
void cpml<T>::initCoefficientArraysXN(short pmlOrder, T alphaOrder, T sigmaMax, T kappaMax, T alphaMax, T epsR, T dt, T dx,
	data3d<T>&Ceyhz, data3d<T>&Cezhy, data3d<T>&Chyez, data3d<T>&Chzey) {
		if (is_cpml_xn) {
			...
			for (unsigned i = 0, iplus = 1; i < n_cpml_xn; i++, iplus++) {
				...
				T alpha_ex = alphaMax * pow(fabs(1 - rho_e), alphaOrder);		
				T alpha_mx = alphaMax * pow(fabs(1 - rho_m), alphaOrder);	
				cpml_b_ex_xn.p[i] = exp((-dt / eps_0) * (sigma_pex / kappa_ex + alpha_ex));
				cpml_b_mx_xn.p[i] = exp((-dt / eps_0) * (sigma_pmx / kappa_mx + alpha_mx));

				cpml_a_ex_xn.p[i] = 1 / dx * (cpml_b_ex_xn.p[i] - 1.0) 
					* sigma_pex / (kappa_ex * (sigma_pex + kappa_ex * alpha_ex));

				cpml_a_mx_xn.p[i] = 1 / dx * (cpml_b_mx_xn.p[i] - 1.0) 
					* sigma_pmx / (kappa_mx * (sigma_pmx + kappa_mx * alpha_mx));
...
template<class T>
void cpml<T>::initCoefficientArraysYN(short pmlOrder, T alphaOrder, T sigmaMax, T kappaMax, T alphaMax, T epsR, T dt, T dy,
	data3d<T>&Cexhz, data3d<T>&Cezhx, data3d<T>&Chxez, data3d<T>&Chzex) {
		if (is_cpml_yn) {
			...
			for (unsigned j = 0, jplus = 1; j < n_cpml_yn; j++, jplus++) {
				...
				T alpha_ey = alphaMax * pow(fabs(1 - rho_e), alphaOrder);
				T alpha_my = alphaMax * pow(fabs(1 - rho_m), alphaOrder);
...
template<class T>
void cpml<T>::initCoefficientArraysYP(short pmlOrder, T alphaOrder, T sigmaMax, T kappaMax, T alphaMax, T epsR, T dt, T dy,
	data3d<T>&Cexhz, data3d<T>&Cezhx, data3d<T>&Chxez, data3d<T>&Chzex) {
		if (is_cpml_yp) {
			T sigmaOpt = sigmaMax * (pmlOrder + 1) / (150 * M_PI * dy * sqrt(epsR));
...
template<class T>
void cpml<T>::initCoefficientArraysZN(short pmlOrder, T alphaOrder, T sigmaMax, T kappaMax, T alphaMax, T epsR, T dt, T dz,
	data3d<T>&Ceyhx, data3d<T>&Cexhy, data3d<T>&Chyex, data3d<T>&Chxey) {
		if (is_cpml_zn) {
			...
			for (unsigned k = 0, iplus = 1; k < n_cpml_zn; k++, iplus++) {
				...
				T alpha_ez = alphaMax * pow(fabs(1 - rho_e), alphaOrder);
				T alpha_mz = alphaMax * pow(fabs(1 - rho_m), alphaOrder);
...
template<class T>
void cpml<T>::initCoefficientArraysZP(short pmlOrder, T alphaOrder, T sigmaMax, T kappaMax, T alphaMax, T epsR, T dt, T dz,
	data3d<T>&Ceyhx, data3d<T>&Cexhy, data3d<T>&Chyex, data3d<T>&Chxey) {
		if (is_cpml_zp) {
			T sigmaOpt = sigmaMax * (pmlOrder + 1) / (150 * M_PI * dz * sqrt(epsR));
